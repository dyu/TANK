...
struct range_base final
{
    struct iterator
    {
        ...
        constexpr iterator(const VT index) : i{ index }
        {
        }
    ...
    constexpr range_base() : offset{ 0 }, len{ 0 }
    {
    }

    constexpr range_base(const range_base &o) : offset(o.offset), len(o.len)
    {
    }

    constexpr range_base(const VT _o, const LT _l) : offset(_o), len(_l)
    {
    }

    // e.g range32_t{10,15}
    constexpr range_base(const std::pair<VT, VT> p) : offset{ p.first }, len{ p.second - p.first }
    {
    }

    constexpr range_base(const LT l) : offset{ 0 }, len{ l }
    {
    }
    ...
    constexpr bool Contains(const VT o) const noexcept
    {
        ...
        return sizeof(VT) == 8 ? o >= offset && o < stop() : uint32_t(o - offset) < len;  // o in [offset, offset+len)
    }
...
{
    return sizeof(VT) == 8 ? v >= s && v < e : uint32_t(v - s) < (e - s);  // o in [offset, offset+len)
};
