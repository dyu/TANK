...
struct simple_allocator
{
    ...
    explicit simple_allocator(simple_allocator &&o) : bankCapacity_(o.bankCapacity_)
    {
        first_ = o.first_;
        last_ = o.last_;
        cur_ = o.cur_;
    ...
    // if you want to make sure that a page holds at least e.g sizeof(foo), use
    // simple_allocator::minBankSizeForSize(sizeof(foo))
    ...
    void allocNewBank_(const uint32_t bs)
    {
        if (cur_ == last_)
        {
            ...
            if (bankCapacity_ & (1u << 31))
            {
                ...
                // > Other products/vendors that recommend to disable THP: cloudera, vertica, varnish, mongo, websphere,
                // maridb, intel, ...
                // Redis now recomends:
                ...
                // > The classical problem with memory allocators is fragmentation. If you allocated a 2MB chunk from
                // the OS
                // > typically using mmap(), as the process runs it's likely some of that 2MB memory block will become
                // free but
                // > not all of it, hence it can't be given back to the OS completely. jamalloc and other allocators use
                // a clever trick
                // > where they use madvise(..., MADV_DONTNEED) to give back portions of memory allocated in such a way
                // back to the OS.
                //
                // > With THP, the OS (and the CPU, really) work with pages of much larger size which only can be
                // unmapped from the address
                // > space in its entirety - which does not work when smaller objects are freed which produce smaller
                // free "holes"
                // > As a result, without being able to free memory efficientl, the mount of allocated memory may go up
                // unbound until
                // > the process starts to swap out - and in the end being killed by the OOMK at least under some
                // workloads.
    ...
    // this is not recommended, however it may be a good idea if the contention is low and if memory fragementation and
