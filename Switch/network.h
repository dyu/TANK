...
inline endpoint ParseSrvEndpoint(strwlen32_t addr, const strwlen8_t proto, const uint16_t srvPort)
{
    ...
    else
    {
        ...
        if (!succ && r.first.len < 250)
        {
            ...
            r.first.ToCString(n);
            if (gethostbyname_r(n, &ret, buf, sizeof(buf), &he, &errno_) == 0 && he && he->h_addrtype == AF_INET &&
                he->h_length)
...
class EPoller
{
  ...
  public:
    EPoller(const uint32_t _maxReturnedEvents = 256) : maxReturnedEvents{ _maxReturnedEvents }
    {
