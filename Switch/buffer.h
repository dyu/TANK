...
class Buffer
{
  ...
  public:  // std::string API
    ...
    Buffer &replace(const uint32_t pos, const uint32_t len, const Buffer &str, const uint32_t subpos,
                    const uint32_t sublen)
    {
    ...
    int compare(const uint32_t pos1, const uint32_t cnt1, const Buffer &str, const uint32_t pos2,
                const uint32_t cnt2) const
    {
  ...
  public:
    ...
    [[gnu::always_inline]] inline uint32_t Reserved() const
    {
        ...
        // UPDATE: we sometimes use SetData() to explicitly set the buffer, and that memory was never allocated from the
        // allocator; it could
        // have been allocated with alloca() or mmap(), and malloc_usable_size() on that ptr would either return bogus
        // values or would result in crashes
    ...
    Buffer(const strwlen32_t content) : Buffer(content.p, content.len)
    {
    }

    Buffer(const strwlen16_t content) : Buffer(content.p, content.len)
    {
    }

    Buffer(const strwlen8_t content) : Buffer(content.p, content.len)
    {
    }

    Buffer(const Buffer &other) : Buffer(other.data(), other.size())
    {
    }
...
#pragma mark Will/Did delegates
    ...
    inline void AppendFmt(const char *fmt, ...) __attribute__((format(printf, 2, 3)));
    ...
    int32_t ReplaceChunk(const uint32_t pos, const uint32_t chunkLen, const char *newChunkData,
                         const uint32_t newChunkLen);
  ...
  public:
    ...
    [[gnu::always_inline]] inline virtual bool OwnsBuffer() const
    {
        ...
        // We need to know if we own the buffer, for otherwise Reserved() will fail (malloc_usable_size(const char*)
        // will abort)
...
class IOBuffer : public Buffer
{
  ...
  public:
    ...
    IOBuffer(const uint32_t len) : Buffer{ len }, position{ 0 }
    {
    }

    IOBuffer(const char *const p, const uint32_t len) : Buffer{ p, len }, position{ 0 }
    {
    }

    IOBuffer(IOBuffer &&b) : Buffer(std::move(b)), position{ b.position }
    {
        b.release();
        b.reset_offset();
    }

    IOBuffer(const IOBuffer &b) : Buffer(b.buffer, b.length_), position{ b.position }
    {
    }
...
struct _srcline_repr
{
    ...
    _srcline_repr(const uint32_t l, const char *const f, const char *const _file) : line{ l }, func{ f }, file{ _file }
    {
    }

    _srcline_repr(const _srcline_repr &o) : line{ o.line }, func{ o.func }, file{ o.file }
    {
    }
...
struct duration_repr
{
    ...
    duration_repr(const uint64_t timeInMicroseconds) : time{ timeInMicroseconds }
    {
    }

    duration_repr(const struct timeval &tv) : time{ uint64_t(tv.tv_sec * 1000000UL + tv.tv_usec) }
    {
    }
    ...
    strwlen8_t Get(char *const out) const
    {
        ...
        {
            len = sprintf(out, "%um%u", (uint32_t)(time / (60 * 1000000UL)),
                          (uint32_t)((time % (60 * 1000000UL)) / 1000000UL));
            while (len && out[len - 1] == '0')
                --len;
            if (len && out[len - 1] != 'm')
                out[len++] = 's';
