...
// This is _very_ useful, for we can check if that's the case and just don't alloc()/copy/free() data instead just point
// to them
...
[[gnu::always_inline]] inline static bool IsConstant(T *const expr)
{
    ...
    else
    {
        ...
        // However, if you use it in an inlined function and pass an argument of the function as the argument to the
        // built-in,
        // GCC never returns when you call the inline function with a string constant or compount literal, and does not
        // return 1 when you
...
struct strwithlen
{
    ...
    inline typename std::enable_if<std::is_same<char, CT>::value, CT *>::type SearchRWithLimit(const CT c,
                                                                                               const LT limit) const
    {
    ...
    constexpr strwithlen() : p{ nullptr }, len{ 0 }
    {
    }

    strwithlen(const char *const s) : p{ s }, len(strlen(s))
    {
    }

    constexpr strwithlen(const char *const s, const char *const e) : p{ s }, len{ LT(e - s) }
    {
    }

    int constexpr Cmp(const CT *const s, const LT l) const
    {
        return l == len ? memcmp(p, s, l) : len < l ? (
                                                              {
                                                                  const auto r = memcmp(p, s, len);
                                                                  r == 0 ? -1 : r;
                                                              }) :
                                                      (
                                                              {
                                                                  const auto r = memcmp(p, s, l);
                                                                  r == 0 ? 1 : r;
                                                              });
    }
    ...
    strwithlen(const CT *const s, const uint32_t l) : p{ s }
    {
    ...
    // It's constexpr
    constexpr strwithlen(const CT *const s, const uint32_t l, const NoMaxLenCheck) : p{ s }, len{ LT(l) }
    {
    }
    ...
    {
        return sizeof(CT) == 8 ? ptr >= p && ptr < (p + len) : uint32_t(ptr - p) < len;
    }
    ...
    double AsDouble() const
    {
        ...
        do
        {
            if (*it == '.' || *it == ',')  // support both radix characters
            {
                ...
                // constexpr uint64_t scale[] = {pow(10, 1), pow(10, 2), pow(10, 3), pow(10, 4), pow(10, 5), pow(10, 6),
                // pow(10, 7), ...
    ...
    typename std::enable_if<std::is_same<char, CT>::value, uint64_t>::type AsUint64() const
    {
        static constexpr uint64_t pow10[20]
                __attribute__((__aligned__(64))) =  // 20 because 20 digits are enough for a 64bit number
                {
                  10000000000000000000ul, 1000000000000000000ul, 100000000000000000ul, 10000000000000000ul,
                  1000000000000000ul, 100000000000000ul, 10000000000000ul, 1000000000000ul, 100000000000ul,
                  10000000000ul, 1000000000ul, 100000000ul, 10000000ul, 1000000ul, 100000ul, 10000ul, 1000ul, 100ul,
                  10ul, 1ul,
                };
    ...
    inline typename std::enable_if<std::is_same<char, CT>::value, bool>::type EqNoCase(const CT *const v,
                                                                                       const LT l) const
    {
    ...
    inline typename std::enable_if<std::is_same<char, CT>::value, bool>::type EndsWithNoCase(const CT *const v,
                                                                                             const LT l) const
    {
        return l <= len && strncasecmp((char *)p + len - l, (char *)v, l) == 0;
    }

    inline typename std::enable_if<std::is_same<char, CT>::value, bool>::type BeginsWith(const CT *const v,
                                                                                         const LT l) const
    {
    ...
    inline typename std::enable_if<std::is_same<char, CT>::value, bool>::type BeginsWithNoCase(const CT *const v,
                                                                                               const LT l) const
    {
    ...
    typename std::enable_if<std::is_same<char, CT>::value, CT *>::type ToCString(CT *const out,
                                                                                 const uint32_t outSize) const
    {
    ...
    LT CommonSuffixLen(const strwithlen o) const
    {
        ...
        for (const auto *oit = oend; oit != op && it != p && oit[-1] == it[-1]; --it, --oit)
        {
            continue;
        }
    ...
    struct _segments
    {
        ...
        struct iterator
        {
            ...
            iterator(const strwithlen input, const CT c) : cur(input), sep(c), next(cur.p)
            {
                Next();
            }
        ...
        _segments(const strwithlen in, const CT c) : s(in), sep(c)
        {
        }
    ...
    struct _segmentsF
    {
        ...
        struct iterator
        {
            ...
            iterator(const strwithlen input, F &lambda) : cur(input), l{ lambda }, next(cur.p)
            {
                Next();
            }
        ...
        _segmentsF(const strwithlen in, F &lambda) : s(in), l(lambda)
        {
        }
