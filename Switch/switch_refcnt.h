...
template <class T>
struct RefCounted : public RCObject
{
  private:
    int32_t rc = 1;
...
struct shared_refptr
{
    ...
    shared_refptr(T *p, const bool) : v{ p }
    {
    }

    shared_refptr(T *p) : v{ p }
    {
        if (v)
            v->Retain();
    }
    ...
    shared_refptr() : v{ nullptr }
    {
    }

    shared_refptr(std::nullptr_t) : v{ nullptr }
    {
    }

    shared_refptr(const shared_refptr &o) : v{ o.v }
    {
        if (v)
            v->Retain();
    }

    shared_refptr(shared_refptr &&o) : v{ o.v }
    {
        o.v = nullptr;
    }
