...
int main(int argc, char *argv[])
{
    ...
    while ((r = getopt(argc, argv, "+vb:t:p:hrS:R:")) != -1)  // see GETOPT(3) for '+' initial character semantics
    {
        switch (r)
        {
            ...
            help:
                ...
                Print("-b broker endpoint: The endpoint of the Tank broker. If not specified, assumed "
                      "localhost:11011\n");
    ...
    const auto consider_fault = [](const TankClient::fault &f)
    {
        switch (f.type)
        {
            case TankClient::fault::Type::BoundaryCheck:
                Print("Boundary Check fault. first available sequence number is ", f.ctx.firstAvailSeqNum,
                      ", high watermark is ", f.ctx.highWaterMark, "\n");
                break;
    ...
    if (cmd.Eq(_S("get")) || cmd.Eq(_S("consume")))
    {
        ...
        while ((r = getopt(argc, argv, "+SF:hBT:KdE:s:")) != -1)
        {
            switch (r)
            {
                ...
                case 'h':
                    ...
                    Print("-F display format: Specify a ',' separated list of message properties to be displayed. "
                          "Properties include: \"seqnum\", \"key\", \"content\", \"ts\". By default, only the content "
                          "is displayed\n");
                    Print("-S: statistics only\n");
                    Print("-E seqNum: Stop at sequence number specified\n");
                    Print("-d: drain and exit. As soon as all available messages have been consumed, exit (i.e do not "
                          "tail)\n");
                    Print("-T: optionally, filter all consumes messages by specifying a time range in either (from,to) "
                          "or (from) format, where the first allows to specify a start and an end date/time and the "
                          "later a start time and no end time. Currently, only one date-time format is supported "
                          "(YYYMMDDHH:MM:SS)\n");
                    Print("\"from\" specifies the first message we are interested in.\n");
                    Print("If from is \"beginning\" or \"start\","
                          " it will start consuming from the first available message in the selected topic. If it is "
                          "\"eof\" or \"end\", it will "
                          "tail the topic for newly produced messages, otherwise it must be an absolute 64bit sequence "
                          "number\n");
                    return 0;
        ...
        for (;;)
        {
            if (!pendingResp)
            {
                ...
                pendingResp =
                        tankClient.consume({ { topicPartition, { next, minFetchSize } } }, drainAndExit ? 0 : 8e3, 0);
    ...
    out:
        if (statsOnly)
            Print(dotnotation_repr(totalMsgs), " messages consumed in ", duration_repr(Timings::Microseconds::Since(b)),
        ...
        while ((r = getopt(argc, argv, "+ho:")) != -1)
        {
            switch (r)
            {
                case 'h':
                    ...
                    Print("Will create a new topic named '", topicPartition.first, "', with total_partitions total "
                                                                                   "partitions\n");
                    Print("Options include:\n");
                    Print("-o config-options: A ',' separated list of key=value options, that will be stored in the "
                          "configuration of the new topic\n");
                    Print("\tSee https://github.com/phaistos-networks/TANK/wiki/Configuration for available "
    ...
    else if (cmd.Eq(_S("mirror")))
    {
        ...
        while ((r = getopt(argc, argv, "+hC:S:z:")) != -1)
        {
            switch (r)
            {
                ...
                case 'h':
                    ...
                    Print("You should have created the partitions(directories) in the destination before you attempt "
                          "to mirror from source to destination.\n");
                    Print("Options include:\n");
                    Print("-C cnt: Each bundle produced will contain no more that `cnt` messages. Default is ",
                          bundleMsgsSetCntThreshold, "\n");
                    Print("-S size: Each bundle produced will be no larger(in terms of bytes) than `size` bytes. "
                          "Default is ",
                          bundleMsgsSetSizeThreshold, "\n");
                    return 0;
        ...
        std::vector<std::pair<TankClient::topic_partition, std::pair<uint64_t, std::vector<TankClient::msg>>>>
                outputsForBase;
        ...
        {
            Print("Partitions mismatch, ", dotnotation_repr(srcPartitionsCnt), " partitions discovered in source, ",
                  dotnotation_repr(pending.size()), " in destination\n");
            return 1;
        }

        Print("Will now mirror ", dotnotation_repr(srcPartitionsCnt), " partitions of ", ansifmt::bold,
              topicPartition.first, ansifmt::reset, "\n");
        Print("You can safely abort mirroring by stoping this tank-cli process (e.g CTRL-C or otherwise). Next mirror "
              "session will pick up mirroring from where this session ended\n");
                        ...
                        if (it.msgs.len)
                        {
                            ...
                            for (const auto m : it.msgs)
                            {
                                if (msgs.size() == bundleMsgsSetCntThreshold || sum > bundleMsgsSetSizeThreshold)
                                {
                                    ...
                                    {
                                        outputsForBase.push_back(
                                                { { topicPartition.first, it.partition }, { first, std::move(msgs) } });
    ...
    else if (cmd.Eq(_S("discover_partitions")))
    {
        ...
        while (tankClient.should_poll())
        {
            ...
            for (const auto &it : tankClient.discovered_partitions())
            {
                ...
                Print(ansifmt::bold, "Partition", ansifmt::set_col(10), "First Available", ansifmt::set_col(30),
                      "Last Assigned", ansifmt::reset, "\n");
                for (const auto wm : it.watermarks)
                    Print(ansifmt::bold, i++, ansifmt::reset, ansifmt::set_col(10), dotnotation_repr(wm->first),
                          ansifmt::set_col(30), dotnotation_repr(wm->second), "\n");
            }
        }
    ...
    else if (cmd.Eq(_S("set")) || cmd.Eq(_S("produce")) || cmd.Eq(_S("publish")))
    {
        ...
        while ((r = getopt(argc, argv, "+s:f:F:hS:K")) != -1)
        {
            switch (r)
            {
                ...
                case 'h':
                    ...
                    Print("Options include:\n");
                    Print("-s number: The bundle size; how many messages to be grouped into a bundle before producing "
                          "that to the broker. Default is 1, which means each new message is published as a single "
                          "bundle\n");
                    Print("-f file: The messages are read from `file`, which is expected to contain the mesasges in "
                          "every new line. The `file` can be \"-\" for stdin. If this option is provided, the messages "
                          "list is ignored\n");
                    Print("-F file: Like '-f file', except that the contents of the file will be stored as a single "
                          "message\n");
                    Print("-K: treat each message in the message list as a key=value pair, instead of as the "
                          "content(value) of a message\n");
                    return 1;
        ...
        if (path[0])
        {
            ...
            if (asSingleMsg)
            {
                ...
                const auto reqId = tankClient.produce({ { topicPartition, msgs } });
        ...
        while ((r = getopt(argc, argv, "+h")) != -1)
        {
            switch (r)
            {
                case 'h':
                    ...
                    Print("Type can be:\n");
                    Print("p2c:  Measures latency when producing from client to broker and consuming(tailing) the "
                          "broker that message\n");
        ...
        if (type.Eq(_S("p2c")))
        {
            ...
            while ((r = getopt(argc, argv, "+hc:s:RB:")) != -1)
            {
                switch (r)
                {
                    ...
                    case 'h':
                        Print("Performs a produce to consumer via Tank latency test. It will produce messages while "
                              "also 'tailing' the selected topic and will measure how long it takes for the messages "
                              "to reach the broker, stored, forwarded to the client and received\n");
                        Print("Options include:\n");
            ...
            while (tankClient.should_poll())
            {
                ...
                if (tankClient.consumed().size())
                {
                    Print("Got data after publishing ", dotnotation_repr(cnt), " message(s) of size ",
                          size_repr(content.len), " (", size_repr(cnt * content.len), "), took ",
        ...
        else if (type.Eq(_S("p2b")))
        {
            ...
            while ((r = getopt(argc, argv, "+hc:s:RB:")) != -1)
            {
                switch (r)
                {
                    ...
                    case 'h':
                        Print("Performs a produce to tank latency test. It will produce messages while also 'tailing' "
                              "the selected topic and will measure how long it takes for the messages to reach the "
                              "broker, stored, and acknowledged to the client\n");
                        Print("Options include:\n");
                        Print("-s message contrent length (default 11 bytes)\n");
                        Print("-c total messages to publish (default 1 message)\n");
                        Print("-R: do not compress bundle\n");
            ...
            Print("Will publish ", dotnotation_repr(cnt), " messages, in batches of ", dotnotation_repr(batchSize),
                  " messages, each message content is ", size_repr(size),
            ...
            while (tankClient.should_poll())
            {
                ...
                {
                    Print("Got ACK after publishing ", dotnotation_repr(cnt), " message(s) of size ",
                          size_repr(content.len), " (", size_repr(cnt * content.len), "), took ",
                          duration_repr(Timings::Microseconds::Since(start)), "\n");
                    return 0;
                }
